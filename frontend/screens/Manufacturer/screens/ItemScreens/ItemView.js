...
export default function ItemView({ route }) {
    ...
    const [itemQAs, setItemQAs] = useState([]);

    useEffect(() => {
        try {
            AsyncStorage.getItem('id').then((value) => {
                ItemQAService.getItemQAByItem(item._id)
                    .then((value) => {
                        setItemQAs(value.data || []);
                    })
                    .catch((error) => {
                        console.error('Error fetching itemQAs:', error);
                    });
            });
        } catch (e) {
            console.log(e);
    ...
    const filteredQA = itemQAs.filter((itemQA) =>
        itemQA.qaName.toLowerCase().includes(searchText.toLowerCase())
    );

    async function deleteItemQA(id) {
        try {
            await ItemQAService.deleteItemQA(id).then((value) => {
